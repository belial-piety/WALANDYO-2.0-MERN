...
exports.voidOrder = asyncHandler(async (req, res, next) => {
  const { voidReason, overrideUsername, overridePassword } = req.body;
  ...
  // Cashiers may request a void, but the action must be approved using the
  // credentials of an active manager assigned to the same branch or an admin.
  if (req.user.role === 'cashier') {
    if (!overrideUsername || !overridePassword) {
      throw new AppError('Manager or administrator override credentials are required.', 403);
    }

    const overrideUser = await User.findOne({
      username: String(overrideUsername).toLowerCase().trim(),
    })
      .select('+passwordHash')
      .populate('branch');

    if (!overrideUser || !(await overrideUser.matchPassword(overridePassword))) {
      throw new AppError('Invalid manager or administrator override credentials.', 401);
    }

    if (!overrideUser.isActive || !['manager', 'admin'].includes(overrideUser.role)) {
      throw new AppError('This account is not authorized to approve transaction voids.', 403);
    }

    if (overrideUser.role === 'manager') {
      if (!overrideUser.branch) {
        throw new AppError('The approving manager is not assigned to a branch.', 403);
      }

      const orderBranchId = existingOrder.branch.toString();
      const managerBranchId = overrideUser.branch._id.toString();
      if (orderBranchId !== managerBranchId) {
        throw new AppError('Manager override must come from a manager assigned to this order branch.', 403);
      }
    }

    authorizer = overrideUser;
  }

  const voided = await orderService.voidOrder(
    req.params.id,
    voidReason,
    req.user,
    authorizer
